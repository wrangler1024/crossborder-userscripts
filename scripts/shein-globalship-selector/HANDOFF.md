...
- 当前正式版本：`0.4.6`
- Git 标签：`shein-globalship-selector-v0.4.6`
- 扩展包：`xynigo-shein-globalship-selector-v0.4.6.zip`
- 支持站点：SHEIN 美国站与墨西哥站桌面版
- 支持路由：搜索页 `/pdsearch/`、类目页 `-c-数字.html`、集合页 `-sc-数字.html`
- 明确不运行：商品详情、购物车、结算、账户和订单页

`0.4.6` 是第二阶段正式里程碑：在原有选品、补全和导出闭环上，增加正式商品网格边界、按页累计、页分组全选、跨页去重、导出进度和与 SHEIN 当前页同步的顺序累计。
...
3. `0.4.6` 发布后只为真实生产回归发布 `0.4.x` 热修复，不在补丁版扩张功能。
