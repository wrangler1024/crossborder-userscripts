...
DEV_DIR="$OUTPUT_DIR/xynigo-dxm-fulfillment-timing-dev"
MODE=${1:---release}
...
PACKAGE_NAME="xynigo-dxm-fulfillment-timing-v$VERSION"
OUTPUT_PATH="$OUTPUT_DIR/$PACKAGE_NAME.zip"

node --check "$SCRIPT_DIR/src/inject.js"
...
copy_extension_files() {
    TARGET_DIR=$1
    mkdir -p "$TARGET_DIR/src" "$TARGET_DIR/icons"
    ...
    cp "$SCRIPT_DIR/src/inject.js" "$TARGET_DIR/src/inject.js"
    cp "$SCRIPT_DIR/src/core.js" "$TARGET_DIR/src/core.js"
    cp "$SCRIPT_DIR/src/content.js" "$TARGET_DIR/src/content.js"
    cp "$SCRIPT_DIR/src/panel.css" "$TARGET_DIR/src/panel.css"
...
if [ "$MODE" = "--release" ] || [ "$MODE" = "--all" ]; then
    TEMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/xynigo-dxm-fulfillment.XXXXXX")
