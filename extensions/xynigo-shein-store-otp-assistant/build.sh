...
PACKAGE_NAME="xynigo-shein-store-otp-assistant-v$VERSION"
OUTPUT_PATH="$OUTPUT_DIR/$PACKAGE_NAME.zip"

node --check "$SCRIPT_DIR/src/otp-core.js"
node --check "$SCRIPT_DIR/src/page-state.js"
...
copy_extension_files() {
    ...
    cp "$SCRIPT_DIR/src/otp-core.js" "$TARGET_DIR/src/otp-core.js"
    cp "$SCRIPT_DIR/src/page-state.js" "$TARGET_DIR/src/page-state.js"
...
if [ "$MODE" = "--dev" ] || [ "$MODE" = "--all" ]; then
    case "$DEV_DIR" in
        "$OUTPUT_DIR"/xynigo-shein-store-otp-assistant-dev) ;;
        *)
...
if [ "$MODE" = "--release" ] || [ "$MODE" = "--all" ]; then
    TEMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/xynigo-shein-otp-extension.XXXXXX")
