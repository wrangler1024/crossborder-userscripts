...
当前正式版本为 `0.5.1`。GitHub Release 提供的 ZIP 用于正式安装；固定 `dev` 目录仅用于本地开发验收。
...
`0.5.1` 本地验收时还应打开工具栏“常用模板”，分别验证一键应用、编辑、覆盖、复制、删除和快捷新建；MX 与 US 站点的模板应分别保存，应用模板不应清空累计商品或已选商品。关闭 SHEIN 标签页或 Comet 后重新打开，当前模板、筛选条件和模板内容都应恢复。
...
- `storage`：把常用模板、当前选中模板和筛选条件保存在扩展本地存储；数据只保留在当前浏览器，不上传到服务器。
