...
test('only mounts on the processing route and removes itself after SPA navigation', async () => {
  ...
  const install = (window) => {
    ...
    window.fetch = async () => { throw new Error('页面范围测试不应发起请求'); };
...
test.skip('legacy failed-order retry blocks tracking mismatches when its page scope is restored', async () => {
  const dom = new JSDOM(`<!doctype html><html><body>
...
test.skip('legacy failed-order retry keeps strict iMile matching when its page scope is restored', async () => {
  const dom = new JSDOM(`<!doctype html><html><body>
    <section class="search-section"><input id="searchContent"><button id="search-button">搜索</button></section>
    <table><tbody id="orders"></tbody></table>
  </body></html>`, {
