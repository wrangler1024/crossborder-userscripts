...
DEV_DIR="$OUTPUT_DIR/xynigo-dxm-logistics-assistant-dev"
MODE=${1:---release}
...
PACKAGE_NAME="xynigo-dxm-logistics-assistant-v$VERSION"
OUTPUT_PATH="$OUTPUT_DIR/$PACKAGE_NAME.zip"

node --check "$SCRIPT_DIR/src/core.js"
node --check "$SCRIPT_DIR/src/import.js"
node --check "$SCRIPT_DIR/src/template-data.js"
node --check "$SCRIPT_DIR/src/content.js"
node --test "$SCRIPT_DIR/tests"/*.test.js

copy_extension_files() {
    TARGET_DIR=$1
    mkdir -p "$TARGET_DIR/src" "$TARGET_DIR/popup" "$TARGET_DIR/icons" "$TARGET_DIR/vendor" "$TARGET_DIR/templates"
    ...
    cp "$SCRIPT_DIR/src/import.js" "$TARGET_DIR/src/import.js"
    cp "$SCRIPT_DIR/src/template-data.js" "$TARGET_DIR/src/template-data.js"
    ...
    cp "$SCRIPT_DIR/vendor/jszip.min.js" "$TARGET_DIR/vendor/jszip.min.js"
    cp "$SCRIPT_DIR/templates/Xynigo店小秘物流助手导入模板.xlsx" "$TARGET_DIR/templates/Xynigo店小秘物流助手导入模板.xlsx"
}
...
if [ "$MODE" = "--release" ] || [ "$MODE" = "--all" ]; then
    TEMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/xynigo-dxm-logistics.XXXXXX")
