...
3. 打开右上角“开发者模式”。
4. 点击“加载已解压的扩展程序”。
5. 选择解压得到的 `xynigo-shein-variant-helper-v0.1.23` 文件夹；该文件夹内应直接看到 `manifest.json`。
...
0.1.22 新增：一行采购链接携带 `pt`（售价快照的 Unix 秒数），供采购售价对比插件展示审单采价时间。原 `op` 仍为优惠券前页面售价；旧版无 `pt` 的链接仍能比价。

如果 Tampermonkey 中已经安装了“Xynigo SHEIN 商品型号助手”，请先停用同名用户脚本，避免两个版本同时在商品页运行。

## 采集避让与手动暂停

妙手忙碌遮罩出现时，型号助手自动撤下界面并停止刷新、点规格和快捷键拦截；待机不会停用。遮罩结束并稳定后自动恢复，旧的待执行动作不会补跑。

面板右上角 **⏸** 可暂停本标签页，刷新后仍暂停；点击“已暂停 · 点击恢复”恢复。该记忆依赖浏览器允许本页 sessionStorage。正常状态仍支持精简链接自动选型。
