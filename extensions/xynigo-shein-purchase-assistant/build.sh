...
PACKAGE_NAME="xynigo-shein-purchase-assistant-v$VERSION"
DEV_DIR="$OUTPUT_DIR/xynigo-shein-purchase-assistant-dev"
...
if [ "$MODE" = "--release" ] || [ "$MODE" = "--all" ]; then
    TEMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/xynigo-shein-purchase-assistant.XXXXXX")
