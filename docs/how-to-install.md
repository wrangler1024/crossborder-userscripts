...
> 第一次使用的同学请按顺序完成下面两步。之后所有脚本都能一键安装，无需重复配置。
...
1. 打开 [仓库主页](../README.md)，在「脚本目录」表格中找到需要的脚本。
2. 点击该行的 **「一键安装」** 链接。
...
本仓库的所有脚本都配置了 `@updateURL`。Tampermonkey 默认每天检查一次更新（可在 Tampermonkey 设置 → 更新 中调整），**仓库推送新版本后你会自动收到更新**，无需手动重装。
